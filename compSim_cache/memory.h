...
protected slots:
    ...
    void resetData();                           // resets the data memory
    ...
    void getCDataAtAddr(word addr);             // gets data at address for cache
    void setCDataAtAddr(word data, word addr);  // sets data at address from cache

signals:
    void returnInst(QString curI);              // returns the instruction requested
    void returnData(word data);                 // returns the data requested

    void returnCData(word data);                // returns the data requested to the cache

    void updateTable(int pos, QString data);    // updates the Memory Table GUI
    void updateITable(int pos, QString data);   // updates the Instruction Table GUI
};
