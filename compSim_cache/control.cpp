...
// Resets the Program Counter and Accumulator
void control::resetPC()
{
    currentPC = 0;
    currentAC = 0;
}
...
void control::exeInstruction(QString inst)
{
    ...
    // LOADA Instruction
    // LOADA
    // Treats the value in the accumulator as an address in data memory
    // and value at this address into the accumulator.
    if (temp.at(0) == "LOADA")
    {
        if (currentAC < 0 || currentAC > 511)
        {
            stopComputer(datamem);
        }
        emit getCData(currentAC);
    }

    // STOREA Instruction
    // STOREA <hex-address>
    // Takes the value in <hex-address> and treats it as an address.
    // Stores the value in the accumulator at the address.
    ...
    if (temp.at(0) == "STOREA")
    {
        ...
        word tempAC = currentAC;
        emit getCData(value);
        if (currentAC < 0 || currentAC > 511)
        {
            stopComputer(datamem);
        }
        //emit setData(currentAC, tempAC);
        emit setCData(tempAC, currentAC);
    ...
    else if (temp.at(0) == "OUT")
    {
        ...
        //word outw = twosComplement(currentAC);
