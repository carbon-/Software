...
void memory::resetMem()
{
    ...
    {
        dataMem[i] = 0;
        updateTable(i, QString("0"));

    }
    for (int i = 0; i < MAX_INST_MEM; i++)
    {
        instMem[i] = "NOP";
        updateITable(i, QString("NOP"));
    }
}

void memory::resetData()
{
    for (int i = 0; i < MAX_DATA_MEM; i++)
    {
        dataMem[i] = 0;
...
// SLOT function to set data at specified address
// Also emits a signal to update the memory table on the GUI
void memory::setCDataAtAddr(word data, word addr)
{
    dataMem[addr] = data;
    QString dataStr = toHexString(data);
    emit updateTable(addr, dataStr);
...
void memory::getCDataAtAddr(word addr)
{
    emit returnCData(dataMem[addr]);
}
